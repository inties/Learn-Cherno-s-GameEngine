...
dll文件包含**所有**的代码和数据，并以导出表记录**对外可见**的函数信息（被声明为__declspec(dllexport)）

**.lib导入表非常轻量**，记录**函数列表和所在动态库名称**



**运行时动态链接**

操作系统根据.lib导入表，去找到相应的动态库和函数，并在dll导出表中找到函数地址，重定向，写到IAT（导入地址表）中。



**区别运行时静态链接**

链接器在编译器就确定了静态链接的所有函数相对位置，运行时只是简单的"基址 + 固定偏移"的计算。而动态链接需要查表（IAT）





dll拥有和exe类似的函数地址搜索机制。如果一个dll A依赖于另一个dll B，而exe依赖于dll A，则exe不需要编译lib B，因为B的函数地址是dll A已知的

 

.lib提供导入表，记录需要的导入动态库和需要导入的函数列表，保证链接通过

.dll提供导出表，并保存函数的二进制代码。

  

IAT导入地址表：记录函数的实际地址。即函数运行时的地址。

运行时，根据导入表加载动态库，并将函数的实际地址记录在IAT中。

 

总之，链接时静态导入库被“编译在”exe或者dll中，定义了依赖关系。

 

**动态链接**通过动态的机制，为上层使用者提供了静态的接口体验
